...
def get_flights():
    ...
    print(page)
    print(size)
    flight_response = requests.get(url=f"http://10.5.0.4:8060/api/v1/flights?page={page}&size={size}")
...
def get_person():
    ...
    tickets_info = requests.get(url=f"http://10.5.0.6:8070/api/v1/tickets/{user}")
    privilege_info = requests.get(url=f"http://10.5.0.5:8050/api/v1/privilege/{user}")
...
def get_ticket(ticketUid: str):
    user = request.headers
    user = user["X-User-Name"]
    ticket_info = requests.get(url=f"http://10.5.0.6:8070/api/v1/tickets/{user}/{ticketUid}")
...
def delete_ticket(ticketUid: str):
    user = request.headers
    user = user["X-User-Name"]
    ticket_info = requests.delete(url=f"http://10.5.0.6:8070/api/v1/tickets/{user}/{ticketUid}")
    if ticket_info.status_code != 200:
        return "Не найден билет", 404
    json_uid = {
        "ticketUid": ticketUid
    }
    status = requests.post(url=f"http://10.5.0.5:8050/api/v1/back_bonuses", json=json_uid,
...
def post_ticket():
    ...
    flight_info = requests.get(url=f'http://10.5.0.4:8060/api/v1/flights/{json_req["flightNumber"]}')
    if flight_info.status_code != 200:
        return "не найден рейс", 404
    json_flight = flight_info.json()

    ticket_info = requests.post(url=f"http://10.5.0.6:8070/api/v1/tickets", json=json_req,
                                headers={"X-User-Name": user})
    ...
    privil_info = requests.post(url=f"http://10.5.0.5:8050/api/v1/buy", json=priv_json_send,
                                headers={"X-User-Name": user})
...
def get_privilege():
    user = request.headers
    user = user["X-User-Name"]
    privilege_info = requests.get(url=f"http://10.5.0.5:8050/api/v1/privileges/{user}")
    return privilege_info.json(), 200


@app.route(f"/api/v1/flights/<ticketUid>", methods=["GET"])
def get_flight_byticket(ticketUid: str):
    req = requests.get(f"http://10.5.0.4:8060/api/v1/flights/{ticketUid}")
    return req.json(), 200
